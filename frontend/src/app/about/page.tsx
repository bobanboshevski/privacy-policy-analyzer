...
const AboutPage = () => {
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 text-white py-16 px-4 md:px-8 relative overflow-hidden">
            ...
            {/* Hero Section */}
            <section className={`text-center max-w-5xl mx-auto mb-20 relative z-10 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
                <div className="relative pb-8">
                    ...
                    {/* Glowing accent */}
                    <div className="absolute -top-4 left-1/2 transform -translate-x-1/2 w-32 h-1 bg-gradient-to-r from-purple-500 to-blue-500 rounded-full blur-sm opacity-60"></div>
                </div>
            ...
            {/* Vision Section */}
            <section className={`max-w-4xl mx-auto text-center mb-20 transition-all duration-1000 delay-500 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
            ...
            {/* Team Section */}
            <section className={`max-w-5xl mx-auto text-center transition-all duration-1000 delay-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
...
const TeamCard = ({ member, index }: { 
    ...
    <div className="bg-white/5 backdrop-blur-xl p-8 rounded-2xl shadow-2xl hover:shadow-purple-500/20 hover:bg-white/10 transition-all duration-500 transform hover:-translate-y-2 border border-white/10 group relative overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-br from-purple-500/10 to-blue-500/10 opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
        <div className="relative z-10">
            <div className={`w-28 h-28 mx-auto rounded-full bg-gradient-to-br ${member.color} flex items-center justify-center mb-6 text-white font-bold text-3xl shadow-lg transform group-hover:scale-110 transition-transform duration-300`}>
                {member.name[0]}
            </div>
            ...
            <div className="flex justify-center space-x-1">
                {[...Array(5)].map((_, i) => (
                    <div key={i} className={`w-2 h-2 rounded-full bg-purple-400 ${i < 3 ? 'opacity-100' : 'opacity-30'}`}></div>
                ))}
            </div>
        </div>
    </div>
